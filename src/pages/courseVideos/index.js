...
export default function KidsCourseContent() {
  ...
  const getData = async () => {
    const course = await axios.post(`${base_url}admin/courses/select_course_data_by_id.php`, {
  ...
  const getStudentProgress = async (video_id) => {
    if (!video_id || !decryptedUserData?.student_id || !decryptedUserData?.token_value) return null;
    try {
      ...
      if (response.data && response.data.status === "success") {
        setTotalPoints(response.data.data.student_answers?.reduce((sum, q) => parseInt(sum) + parseInt(q.points || 0), 0));
      }
      return null;
    } catch (err) {
  ...
  const getCourseData = async () => {
    setPageLoading(true);
    const response = await axios.post(`${base_url}user/courses/select_course_units.php`, {
      course_id: course_id.get("course_id"),
      student_id: decryptedUserData?.student_id,
      token_value: decryptedUserData?.token_value,
    });
    console.log(response);
    setCourseData({ course_name: course_id?.get("course_name"), units: response.data.message });
  ...
  const getVideoQuestions = async (videoId) => {
    const response = await axios.post(`${base_url}user/interactive_videos/get_video_questions.php`, {
      video_id: videoId,
      student_id: decryptedUserData?.student_id,
      token_value: decryptedUserData?.token_value,
    });
    ...
    const formattedQuestions = response?.data?.message && Array.isArray(response?.data?.message) && (response.data.message || []).map((q) => {
      ...
      if (Array.isArray(q.real_answers) && q.real_answers.length > 0) {
        ...
        answers = q.question_answers.split("//CAMP//").map((ans) => {
      ...
      if (Array.isArray(q.real_answers)) {
        const correct = q.real_answers.find((ans) => ans.answer_check === true);
        if (correct) {
      ...
      return {
        ...
        explanation: Array.isArray(q.real_answers)
          ? (q.real_answers.find((ans) => ans.answer_check) || q.real_answers.find((ans) => ans.answer_exp && ans.answer_exp.trim()))?.answer_exp || ""
          : "",
  ...
  useEffect(() => {
    if (courseData && courseData?.units?.length > 0) {
      const videoId = courseData?.units?.[currentTab]?.videos?.[currentIndex]?.source_video_id;
  ...
  const handleSubmitAnswer = async (question) => {
    ...
    try {
      ...
      await axios.post(`${base_url}/user/interactive_videos/submit_answer.php`, {
        student_id: decryptedUserData?.student_id,
        token_value: decryptedUserData?.token_value,
  ...
  return (
    ...
    >
      <SEO title={currentVideo?.name ? `الفيديو: ${currentVideo?.name}` : 'مشاهدة الدرس'} lang="ar" />
      <QuizModal
      ...
      <div className="!max-w-[100%] !mx-auto !px-1 sm:!px-4 !py-2 sm:!py-2">
        <div className="!grid !grid-cols-1 lg:!grid-cols-3 !gap-4 sm:!gap-8">
          <div className="lg:!col-span-2">
            <div className="!bg-white !rounded-2xl sm:!rounded-3xl !shadow-2xl !overflow-hidden !transform !transition-all !duration-500 !border-none !max-h-[125vh]">
              <div className="!bg-gradient-to-r !from-blue-600 !via-red-500 !to-indigo-600 !px-2 !py-2 !animate-gradient-x">
                <div className="!flex !min-w-[100%] !items-center !justify-between">
                  <div className="!flex !items-center !text-white">
                    <Play className="!ml-2 sm:!ml-4 !text-2xl sm:!text-3xl" />
                    <div>
                  ...
                  <button
                    ...
                    style={{
                      ...
                      borderRadius: '999px',
                      background:
                        'linear-gradient(90deg, #4f8cff 0%, #a084ee 100%)',
                      color: '#fff',
                      fontWeight: 700,
                      fontSize: '1.2rem',
                      boxShadow: '0 4px 24px #a084ee55',
                      border: 'none',
                      cursor: 'pointer',
...
{console.log("currentVideo?.loom_url", currentVideo?.loom_url)}
          ...
          {showContent && (
            <div className="lg:!col-span-1">
              <div className="!bg-white !rounded-2xl sm:!rounded-3xl !shadow-2xl !overflow-auto !transform !transition-all !duration-500 !border-none !max-h-[50vh] lg:!max-h-[125vh]">
                ...
                <div className="!max-h-[100%] !overflow-y-auto">
                  ...
                  {courseData && courseData.units && courseData.units.length > 0 && courseData.units.map((unit, unitIndex) => (
                    <div key={unit.id} className="!border-b !border-gray-200">
                      ...
                      <div className="!px-3 sm:!px-6 !py-2 sm:!py-4">
                        {unit.videos && unit.videos.length > 0 && unit.videos.map((video, videoIndex) => (
                          <button
                            key={video.id}
                            onClick={() =>
                            ...
                            <div className="!flex !items-center !justify-between">
                              <div className="!flex !items-center">
                                ...
                                <div>
                                  ...
                                  >
                                    ⏱️ {video.video_duration ? video.video_duration : "00:00"}
